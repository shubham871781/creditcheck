...
export default function Searches(props) {
  ...
  const { t } = useTranslation();
    const handleClose11 = () => setSearchModal(false);
    const handleShow11 = () => {
      setSearchModal(true);
      searchapi();
    }
    ...
    const searchapi = async () => {
        try {
          const response = await fetch('http://localhost:5000/reportapi/Search_value', {
            method: "post",
            headers: {
            ...
            .then(data => {
              setSearch_data(data.result.items) 
  ...
  return (
    ...
    <div>
         <p className="report_head" onClick={handleShow11}>12. {t("SEARCHES")}</p>
      ...
      <Modal show={searchModal} onHide={handleClose11} dialogClassName="modal-new1" size="lg">
                <Modal.Header closeButton>
                  <Modal.Title> {t("SEARCHES")}</Modal.Title>
                </Modal.Header>
                <Modal.Body >
